...
class RNNCell(object):
    ...
    def forward(self, x, h):
        ...
        """
        h_prime = self.activation(x@self.W_ih.T + self.b_ih 
                                  + h@self.W_hh.T + self.b_hh)
        return h_prime
    ...
    def backward(self, delta, h, h_prev_l, h_prev_t):
        ...
        self.db_ih += np.sum(dz, axis=0).T/batch_size
        self.db_hh += np.sum(dz, axis=0).T/batch_size
