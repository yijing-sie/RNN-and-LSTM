...
class RNNPhonemeClassifier(object):
    ...
    def forward(self, x, h_0=None):
        ...
        for i in range(seq_len):
            hidden_unit = self.hiddens[i]
            input_x = self.x[:,i,:]
            out = []
            for j, rnn_cell in enumerate(self.rnn):
                out += [rnn_cell(input_x , hidden_unit[j])]
                input_x = out[-1]
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        for i in range(seq_len - 1, -1, -1):
            for j in range(self.num_layers - 1, -1, -1):
                h = self.hiddens[i + 1][j]
                if j !=0:
                    h_prev_l = self.hiddens[i + 1][j - 1]  
                else : h_prev_l = self.x[:, i, :]                
                h_prev_t = self.hiddens[i][j]                
                (out_dx, out_dh)= self.rnn[j].backward(dh[j], h, h_prev_l, h_prev_t)
                dh[j] = out_dh
                if j != 0: dh[j - 1] += out_dx
        dh = dh/batch_size                
        return dh
